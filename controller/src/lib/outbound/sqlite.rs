...
impl MetricsRepository for Sqlite {
    async fn add_metrics(&self, req: AddMetricsRequest) {
        ...
        for (service_name, _sample) in &req.samples {
            ...
            .bind(_sample.disk_read_bytes as i64)
            .bind(_sample.disk_write_bytes as i64)
            .bind(&req.user_id)
...
impl AlertsRepository for Sqlite {
    async fn list_rules(&self, user_id: &str) -> Result<Vec<AlertRule>, AlertRuleError> {
        let rows = sqlx::query(
            "SELECT id, user_id, metric, threshold, for_seconds, cooldown_seconds,
                    hostname, project, service, enabled, created_at
                FROM alert_rule
                WHERE user_id = ?
                ORDER BY created_at DESC, id DESC",
        )
    ...
    async fn delete_rule(
        ...
        let result = sqlx::query("DELETE FROM alert_rule WHERE id = ? AND user_id = ?")
            .bind(rule_id)
            .bind(user_id)
            .execute(&self.pool)
            .await
    ...
    async fn set_enabled(
        ...
        let result = sqlx::query("UPDATE alert_rule SET enabled = ? WHERE id = ? AND user_id = ?")
    ...
    async fn get_states(
        ...
        // sqlite can't bind an array; the id count is bounded by the per-user
        // rule cap, so an expanded placeholder list is fine.
        let placeholders = vec!["?"; rule_ids.len()].join(", ");
        let sql = format!(
            "SELECT rule_id, service, state FROM alert_state
                WHERE hostname = ? AND project = ? AND rule_id IN ({placeholders})"
        );
