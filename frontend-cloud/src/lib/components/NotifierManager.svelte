<script lang="ts">
	...
	import type { Notifier, NotifierKind } from '$lib/api/notifiers';
	import type { PlanStatus } from '$lib/api/me';

	/** Result of the hosting page's notifier form actions. */
	type NotifierFormResult = {
		createError?: string;
		deleteError?: string;
		toggleError?: string;
		testError?: string;
	...
	// The create/toggle/test/delete forms post to the hosting page's `?/create`,
	// `?/toggle`, `?/test` and `?/delete` actions (see `$lib/server/notifierActions`).
	let {
		notifiers,
		me,
		form,
		error = null,
		slackHref,
		listTitle = 'Your notifiers',
		emptyText = 'No notifiers yet. Add one above and your next deployment will ping you.'
	}: {
		notifiers: Notifier[];
		me: PlanStatus | null;
		form: NotifierFormResult | undefined;
		error?: string | null;
		slackHref: string;
		listTitle?: string;
		emptyText?: string;
	...
	const allowed = $derived<Set<NotifierKind>>(
		new Set(me?.limits.allowed_notifier_kinds ?? ['slack', ...ALL_KINDS.map((k) => k.value)])
	);
	const isFree = $derived((me?.plan ?? 'free') === 'free');
	...
	function summarise(n: Notifier): string {
...
{:else if form?.testedId}
	<div class="rounded-xl border border-success-border bg-success-bg px-4 py-3 text-sm text-success">
...
{#if slackEnabled}
	...
	<section class="rounded-xl border border-line bg-card p-5">
		...
		<p class="mb-3 text-sm text-ink-muted">
			Install Hoister into a Slack channel in two clicks — pick a channel and we'll post deployment
			events there. No webhook URLs to copy.
		</p>
		{#if slackAllowed}
			<a
				href={slackHref}
		...
		{:else if kind === 'teams'}
			<div class="space-y-2">
				...
				<p class="text-xs text-ink-faint">
					...
					→ copy the generated URL. No app registration needed; messages post as the webhook.
				</p>
			</div>
		{:else if kind === 'gotify'}
			<div class="grid gap-3 sm:grid-cols-2">
