...
export function parseConfig(form: FormData): NotifierConfig | { error: string } {
...
/**
 * The create / delete / toggle / test form actions behind `NotifierManager`,
 * shared by the account-wide notifiers page and the per-project one.
 * `projectOf` picks the scope: `undefined` for account-wide notifiers, or the
 * id of the project whose notifiers are managed (the controller checks that
 * the user has access to it).
 */
export function notifierActions(projectOf: (event: RequestEvent) => string | undefined) {
	return {
		create: async (event: RequestEvent) => {
			...
			try {
				const result = await createNotifier(auth.userId, parsed, projectOf(event));
				if (!result.ok) {
		...
		delete: async (event: RequestEvent) => {
			...
			try {
				const ok = await deleteNotifier(auth.userId, id, projectOf(event));
		...
		toggle: async (event: RequestEvent) => {
			...
			try {
				const ok = await setNotifierEnabled(auth.userId, id, enabled, projectOf(event));
		...
		test: async (event: RequestEvent) => {
			...
			try {
				const result = await testNotifier(auth.userId, id, projectOf(event));
